 ...
 */
typedef struct {
    GoengcBitfield occupied_bits; /* 1 = stone present (black/white),
                                     0 = empty/off-board */
 ...
 */
static inline void goengc_colorfield_fill(GoengcColorField* restrict field,
                                          GoengcColor color) {
    assert(field != NULL);
 ...
 */
static inline GoengcColor goengc_colorfield_get_color(
    const GoengcColorField* restrict field, uint16_t index) {
    assert(field != NULL);
 ...
 */
static inline void goengc_colorfield_set_color(GoengcColorField* restrict field,
                                               uint16_t index,
                                               GoengcColor color) {
    assert(field != NULL);
    assert(index < GOENGC_DATA_SIZE_SQUARED);
